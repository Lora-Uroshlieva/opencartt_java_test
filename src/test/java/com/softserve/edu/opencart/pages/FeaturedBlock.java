...
public class FeaturedBlock {
    ...
    public List<ProductComponent> initProductComponents() {
    ...
    public boolean checkClassPresenceInElements(String className) {
        boolean result = true;
        for (ProductComponent currentEl: initProductComponents()) {
            if(!currentEl.getElementClasses().contains(className)) {
                result = false;
                break;
            }
        }
